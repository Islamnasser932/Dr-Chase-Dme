...
                "Approval date": lambda x: x.notna().sum(),
                "Denial Date": lambda x: x.notna().sum(),
Â               "Completion Date": lambda x: x.notna().sum(),
                "Upload Date": lambda x: x.notna().sum(),
            }).reset_index()
        
...
I                    with st.expander("🔍 View Leads Missing Completion Date after Upload"):
                        st.dataframe(
                            bad_uploaded[["MCN", "Client", "Chaser Name", "Upload Date", "Completion Date"]],
D                            use_container_width=True
                        )
            
...
Source                if not bad_uploaded_assigned.empty:
                    st.warning(f"⚠️ Found {len(bad_uploaded_assigned)} leads with **Upload Date** but no **Assigned date**.")
See                    with st.expander("🔍 View Leads Missing Assigned Date after Upload"):
                        st.dataframe(
...
s                    with st.expander("🔍 View Leads Missing Approval Date after Upload"):
                        st.dataframe(
...
C            avg_denial_age = df_lead_age["Lead Age (Denial)"].mean(skipna=True)
        
...
Indented                            tooltip=["Category", "Count"]
                        )
                    )
...
                    weeks_negative = sorted([c for c in categories if "Week -" in c], key=lambda x: int(x.split()[1]))
Note                    weeks_positive = sorted([c for c in categories if "Week " in c and "-" not in c], key=lambda x: int(x.split()[1]))
                    category_order = weeks_negative + weeks_positive
        
                    denial_summary = (
...
                    D       df_lead_age["Denial Category"].astype(str).str.contains("Week -", na=False)
                        ]
...
    Note          st.markdown("### 📊 Approval vs Denial Lead Age by Client")
...
Indented        st.subheader("🔍 Duplicate Leads by MCN (Considering Product)")
        
...
Note                available_dup_cols = [c for c in required_cols if c in dup_same_product.columns]
                
                # **الجدول المطلوب: Duplicate Leads (MCN & Product) Details**
                st.markdown("### 📋 Duplicate Leads (MCN & Product) Details")
                st.dataframe(
      s               dup_same_product.sort_values(["MCN", "Products", "Created Time"])[available_dup_cols],
                    use_container_width=True
                )
                
                # 📌 تم حذف الجدول: "📊 Duplicate MCN (Same Product) Grouped by Key Dates"source
        
            else:
...
Note                        "Assigned date (Date)","Approval date (Date)","Denial Date (Date)",
                        "Completion Date (Date)","Upload Date (Date)","Client",
                        "Chasing Disposition","Insurance","Type Of Sale"
                    ]
                  T    available_cols_for_diff_dups = [c for c in cols_to_show_old if c in dup_diff_product.columns]
                     
                Type       merged = dup_diff_product.merge(dup_diff_product_grouped[["MCN"]], on="MCN")
                     st.dataframe(
